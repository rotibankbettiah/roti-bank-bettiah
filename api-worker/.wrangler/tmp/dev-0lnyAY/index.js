...
// .wrangler/tmp/bundle-lomBAY/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-lomBAY/middleware-insertion-facade.js
init_modules_watch_stub();
...
var createOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const receipt = `receipt_${Date.now()}_${Math.floor(Math.random() * 1e3)}`;
    const payload = {
...
var verifyPayment = /* @__PURE__ */ __name(async (c) => {
  try {
    const body = await c.req.json();
...
`;
var generateChatResponse = /* @__PURE__ */ __name(async (userPrompt, apiKey) => {
  try {
    if (!apiKey) {
...
app.post("/api/chat", async (c) => {
  try {
    ...
    if (message.trim().length === 0 || message.length > 1e3) {
